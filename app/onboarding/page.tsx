  ...
  const steps = [
      ...
      component: (
        <div className="text-center">
          ...
          <div className="bg-blue-50 p-6 rounded-lg max-w-lg mx-auto">
            <h4 className="font-semibold text-blue-900 mb-2">What makes us different:</h4>
      ...
      component: (
        <div className="grid md:grid-cols-2 gap-4">
          {[
            ...
            { value: 'build-projects', label: 'Build Personal Projects', icon: '🛠️' },
            { value: 'improve-skills', label: 'Improve Current Skills', icon: '📈' },
            { value: 'start-business', label: 'Start a Tech Business', icon: '🚀' },
    ...
    },
    {
      ...
      component: (
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { value: 'html-css', label: 'HTML & CSS', icon: '🌐', desc: 'Web fundamentals' },
            { value: 'javascript', label: 'JavaScript', icon: '⚡', desc: 'Core programming' },
            { value: 'react', label: 'React', icon: '⚛️', desc: 'Frontend framework' },
            { value: 'typescript', label: 'TypeScript', icon: '📘', desc: 'Type-safe JS' },
            { value: 'nodejs', label: 'Node.js', icon: '🟢', desc: 'Backend development' },
            { value: 'python', label: 'Python', icon: '🐍', desc: 'General purpose' },
            { value: 'databases', label: 'Databases', icon: '🗄️', desc: 'Data management' },
            { value: 'devops', label: 'DevOps', icon: '⚙️', desc: 'Deployment & tools' },
            { value: 'mobile', label: 'Mobile Dev', icon: '📱', desc: 'App development' }
          ].map((tech) => (
            <Card 
              key={tech.value}
              className={`cursor-pointer transition-all duration-200 ${
              ...
              onClick={() => toggleArrayValue('preferredTechnologies', tech.value)}
            >
              <CardContent className="p-4 text-center">
                <div className="text-2xl mb-2">{tech.icon}</div>
                <h4 className="font-semibold text-gray-900 text-sm">{tech.label}</h4>
    ...
    },
    {
      ...
      component: (
        <div className="space-y-4">
          {[
            { value: '30-min', label: '30 minutes per day', desc: 'Perfect for building consistent habits' },
      ...
      component: (
        <div className="text-center">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-6 rounded-lg max-w-2xl mx-auto mb-8">
            <h4 className="font-semibold text-gray-900 mb-4">Your Personalized Plan:</h4>
            <div className="grid md:grid-cols-2 gap-4 text-left">
              <div>
                <h5 className="font-medium text-gray-800 mb-2">Experience Level:</h5>
                <Badge variant="info">{formData.experience.replace('-', ' ')}</Badge>
              </div>
              <div>
                <h5 className="font-medium text-gray-800 mb-2">Time Commitment:</h5>
                <Badge variant="success">{formData.timeCommitment}</Badge>
              </div>
              <div>
                <h5 className="font-medium text-gray-800 mb-2">Primary Goals:</h5>
                <div className="flex flex-wrap gap-1">
                  {formData.goals.slice(0, 2).map((goal, index) => (
                    <Badge key={index} variant="warning" size="sm">{goal.replace('-', ' ')}</Badge>
                  ))}
                </div>
              </div>
              <div>
                <h5 className="font-medium text-gray-800 mb-2">Focus Technologies:</h5>
                <div className="flex flex-wrap gap-1">
                  {formData.preferredTechnologies.slice(0, 3).map((tech, index) => (
                    <Badge key={index} variant="purple" size="sm">{tech.replace('-', ' ')}</Badge>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="bg-yellow-50 p-4 rounded-lg max-w-lg mx-auto">
            <p className="text-yellow-800 font-medium">
              🍪 Get ready for some celebration confetti when you complete challenges!
            </p>
          </div>
        </div>
      )
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex justify-between max-w-4xl mx-auto">
          ...
          <Button 
            onClick={handleNext}
            className={currentStep === totalSteps - 1 ? 'bg-green-600 hover:bg-green-700' : ''}
          >
